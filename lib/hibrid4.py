...
def feature_engineering(data, SPY, predictions=np.array([None]))->pd.core.frame.DataFrame:
    ...
    if predictions.any() == True:
        data = yf.download("BTC-USD", start="2001-11-30")
...
def train_xgb_model(X_train, y_train, X_val, y_val, plotting=False):
    ...
    if plotting:
        ...
        plt.xlabel("Time")
        plt.ylabel("BTC-USD stock price")
...
def create_chart(symbol, days=7, entry=None, tp1=None, tp2=None, stop_loss=None, trade_direction=None):
    print(f"Iniciando creación del gráfico para {symbol}")
    end_date = datetime.now(pytz.UTC)
    start_date = end_date - timedelta(days=days)
    data = yf.download(symbol, start=start_date, end=end_date)
...
def main():
    ...
    try:
        ...
        historical_prices = get_historical_prices(symbol, days=7)
        ...
        if not historical_prices.empty and len(historical_prices) >= 2:
            ...
            chart_path = create_chart(symbol, days=7, entry=entry, tp1=tp1, tp2=tp2, stop_loss=stop_loss, trade_direction=trade_direction)
            if chart_path:
            ...
            print(f"Datos históricos disponibles: {len(historical_prices)} días")

        current_date = datetime.now().strftime('%Y-%m-%d')
        current_time = datetime.now().strftime('%H:%M')
        
        message = f"""
*BTC-USD Prediction* para {current_date}
